...
defmodule Leader do
    ...
    def next(s) do
        ...
        receive do
            ...
            {:APPEND_REPLY, pid, term, success, index} ->
                s =
                    if term > s.curr_term do
                        s = Follower.stepdown(s, term)
                        Follower.start(s)
    ...
    def send_append_entry(s, server) do
        ...
        # IO.puts "prevlogindex #{inspect server} #{prev_log_index}"
        # IO.puts "#{inspect server} #{Map.get(s.next_index, server)} #{length(s.log) - 1}"
    ...
    def check_new_commit_index(s) do
        ...
        # IO.puts "match_index #{s.commit_index} #{inspect match_indexes}"
        s
    end
